...
Use current stable dependency versions when implementation starts and commit the pnpm lockfile. Do not introduce SSR without a concrete later requirement; V1 is a client-side SPA.
...
- pnpm install
- pnpm dev — full local SPA/API against local D1
- pnpm db:migrate — apply local migrations
- pnpm db:seed — load canonical deterministic demo data
- pnpm db:reset — reset/recreate local DB and seed it
- pnpm test — unit/integration tests
- pnpm test:e2e — browser tests
- pnpm lint
- pnpm typecheck
- pnpm build
- pnpm check — non-interactive aggregate quality gate

Common local development must not require manual Cloudflare dashboard work or interactive login. Seed data must never be automatically inserted into production.
...
Deployment is separate from ordinary CI unless explicitly configured later.
